...
export const useSetupWizardStore = create<SetupWizardState>((set, get) => ({
  ...
  loadAllData: async () => {
    ...
    try {
      ...
      if (company) {
        ...
        {/*console.log('[DEBUG] loadAllData - Loading existing data:', {
          divisions: divisions,
          clusters: clusters,
          existingPendingDivisions: existingPendingDivisions,
          existingPendingClusters: existingPendingClusters
        });*/}
  ...
  completeSetup: async () => {
    ...
    try {
      ...
      if (response.ok) {
        console.log('[SetupWizard] Setup completed successfully, redirecting to forecast...');
  ...
  mapCsvColumns: async (divisionColumn: string | null, clusterColumn: string | null, lifecycleColumn: string | null) => {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('[DEBUG] Backend extract result:', result);
        ...
        if (result.extractedClusters.length > 0 && result.divisionClusterMap) {
          ...
          console.log('[DEBUG] Consistent ordering result:', consistentOrdering);
          ...
          consistentOrdering.clusters.forEach((clusterInOrdering) => {
            ...
            console.log(`[DEBUG] Added cluster "${clusterInOrdering.name}" with consistent division ID: ${clusterInOrdering.divisionId}`);
          });
        }
  ...
  addPendingCluster: (cluster) => {
    ...
    console.log(`[DEBUG] Adding cluster "${cluster.name}" to pending clusters in division "${divisionName}"`);
  ...
  createAllPendingItems: async () => {
    ...
    try {
      ...
      if (newPendingClusters.length > 0) {
        ...
        console.log(`[DEBUG] Division name to ID mapping:`, divisionNameToId);
        console.log(`[DEBUG] Processing ${newPendingClusters.length} clusters:`, newPendingClusters.map(c => ({ name: c.name, divisionId: c.divisionId, divisionName: c.divisionName })));
        ...
        const resolvedClusters = newPendingClusters.map(cluster => {
          ...
          console.log(`[DEBUG] Resolving cluster "${cluster.name}":`, {
            originalDivisionId: cluster.divisionId,
            divisionName: cluster.divisionName,
            divisionNameToId: divisionNameToId
          });
          ...
          if (cluster.divisionId >= 0) {
            ...
            if (cluster.divisionId < consistentOrdering.divisions.length) {
              ...
              console.log(`[DEBUG] Resolved cluster "${cluster.name}" using consistent index ${cluster.divisionId} to division "${targetDivision.name}" with ID: ${resolvedDivisionId}`);
            } else {
          ...
          console.log(`[DEBUG] Final resolved cluster "${cluster.name}":`, {
            originalDivisionId: cluster.divisionId,
            resolvedDivisionId,
            divisionName: cluster.divisionName
          });
  ...
  calculateSetupFlow: async () => {
    ...
    {/*console.log('[DEBUG] calculateSetupFlow called with:', {
      hasMultipleDivisions,
      hasMultipleClusters,
      importLevel,
      divisionCsvType,
    ...
    {/*console.log('[DEBUG] Final setup flow configuration:', {
      skipDivisionStep,
      skipClusterStep,
      requiresCsvUpload,
      divisionValue,
